...
public class LIS2Java implements Constants {
            ...
            if (!inarr(ops, op) && !inarr(small, op)) {
                ...
                if (nextchar != '(') {
                    if (!invec(vars, op)) {
                        ...
                        if (!invec(varsdiff, opdiff) && !invec(dimdiff, opdiff)) {
                            varsdiff.add(opdiff);
                        } else {
    ...
    private int parseCommand(int keyword, int linepos, int lexpos) {
        ...
        switch (keyword) {
            ...
            case tokEQUALS:
                if (!ifEnded) {
                    if ((getType(linepos, lexpos - 1).equals(TYPE_VARIABLE) && getLex(linepos, lexpos - 1).endsWith("$")) || getType(linepos, lexpos - 1).equals(TYPE_STRING)) {
                        mainCode += ".equals(";
                        equalsBracket = true;
                    } else {
                        mainCode += " == ";
            ...
            case tokEDITFORM:
                mainCode += "_EditForm(";
                lexpos = parseRValue(linepos, lexpos, 7);
                mainCode += ")";
                break;
            case tokGAUGEFORM:
                break;
            case tokCHOICEFORM:
                mainCode += "_ChoiceForm(";
                needArray = 5;
                lexpos = parseRValue(linepos, lexpos, 6);
                mainCode += ")";
                break;
            case tokDATEFORM:
                ...
                break;
            case tokMESSAGEFORM:
                mainCode += "_MessageForm(";
                lexpos = parseRValue(linepos, lexpos, 5);
                mainCode += ")";
                break;
            case tokLOG:
                mainCode += "_log((float) ";
